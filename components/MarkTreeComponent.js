...
class MarkTreeComponent extends React.Component {
  constructor(props) {
    super(props) 
    this.state = {
      replyOpen: false,
  ...
  async replyComment (e) {  
    ...
    const body = JSON.stringify({ markdownID: this.props.markdownID, comment: userreply, parentID: this.props.node._id, postID: this.props.node.postID, markdownuserID: this.props.markdownuserID });
    
    try {
        
        const res = await axios.put(`/api/markdown/comments/${this.props.node.postID}`, body, config); 
        console.log(res)
  ...
  render() {
    ...
    if (this.props.node.children && this.props.node.children.length > 0) {
      childNodes = this.props.node.children.map((node, index) => {
        return <li key={index}><MarkTreeComponent onAddComment={this.props.onAddComment} markdownID={this.props.markdownID} loggedinuser={this.props.loggedinuser} node={node} token={this.props.token} markdownuserID={this.props.markdownuserID} /></li>
      });
    }
...
export default MarkTreeComponent;
...
class MarkTreeComponent extends React.Component {
  ...
  async replyComment (e) {  
    ...
    const body = JSON.stringify({ markdownID: this.props.markdownID, comment: userreply, parentID: this.props.node._id, postID: this.props.node.postID});
  ...
  render() {
    ...
    if (this.props.node.children && this.props.node.children.length > 0) {
      childNodes = this.props.node.children.map((node, index) => {
        return <li key={index}><MarkTreeComponent onAddComment={this.props.onAddComment} markdownID={this.props.markdownID} loggedinuser={this.props.loggedinuser} node={node} /></li>
      });
    }

    const { replyOpen, userreply } = this.state

    console.log(this.props.markdownID)
